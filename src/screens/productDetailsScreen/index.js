...
const ProductDetailsScreen = () => {
  ...
  const receiverId = storeData?.contact?.id;
  ...
  useEffect(() => {
    if (storeId) {
      ...
      console.log('Get Store Id', storeId);
    }
  }, [storeId]);
  ...
  const cartItems = useSelector(
    state => state.addToCard?.cartData?.productDetailList || EMPTY_ARRAY,
  );
  // const cartItems = useSelector(
  //   state => state.addToCard?.cartData?.productDetailList || [],
  // );
  ...
  const handleRemoveFromCart = () => {
    ...
    console.log('ITEM ID =>', cartItem?._id);
          ...
          <View style={{width: hp(50), height: hp(50)}}>
            ...
            <TouchableOpacity
              ...
              {isInWishlist(productId) ? (
                <SavedFiledIcon width={wp(23)} height={hp(20)} />
              ) : (
                <Blackheart width={wp(23)} height={hp(20)} />
              )}
            </TouchableOpacity>
          </View>
        </View>
